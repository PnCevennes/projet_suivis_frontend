angular.module('FormDirectives').directive('fileinput', function(){
    return {
        ...
        controller: ['$scope',
            ...
            $scope.init_object = function() {
                $scope.default_object = {
            ...
            $scope.$watch(function(){return $scope.refer}, function(nv, ov){
                if(nv !== ov && (!!ov && ov.length >0)  && (!!nv && nv.length >0)){
                    $scope.refer = nv || [angular.copy($scope.default_object)];
                    if(!$scope.refer.length){
                        $scope.refer.push(angular.copy($scope.default_object));
                    }
                    $scope.items = angular.copy($scope.refer);
                }
            });
            ...
            $scope.reset_item = function(idx){
                if($scope.items[idx]){
                    ...
                    $scope.refer[idx] = angular.copy($scope.default_object);
                }
            }
